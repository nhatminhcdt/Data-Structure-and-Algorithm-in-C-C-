/***
 * 
 * @brief [Task description](https://app.codility.com/programmers/lessons/5-prefix_sums/genomic_range_query/)
 * 
...
vector<int> solution(string &S, vector<int> &P, vector<int> &Q) {
  ...
  std::vector<int> prefix_sum_A(N + 1, 0);
  std::vector<int> prefix_sum_C(N + 1, 0);
  std::vector<int> prefix_sum_G(N + 1, 0);
  std::vector<int> prefix_sum_T(N + 1, 0);

  for (int i = 0; i < N; ++i) {
    prefix_sum_A[i + 1] = prefix_sum_A[i] + static_cast<int>(S[i] == 'A');
    prefix_sum_C[i + 1] = prefix_sum_C[i] + static_cast<int>(S[i] == 'C');
    prefix_sum_G[i + 1] = prefix_sum_G[i] + static_cast<int>(S[i] == 'G');
    prefix_sum_T[i + 1] = prefix_sum_T[i] + static_cast<int>(S[i] == 'T');
  }
  ...
  for (unsigned int k = 0; k < P.size(); ++k) {
    ...
    // Check prefix sums to determine the minimal impact factor
    if (prefix_sum_A[end] - prefix_sum_A[start] > 0) {
      result.push_back(IMPACT_A);
    } else if (prefix_sum_C[end] - prefix_sum_C[start] > 0) {
      result.push_back(IMPACT_C);
    } else if (prefix_sum_G[end] - prefix_sum_G[start] > 0) {
      result.push_back(IMPACT_G);
    } else if (prefix_sum_T[end] - prefix_sum_T[start] > 0) {
      result.push_back(IMPACT_T);
    }
  }
