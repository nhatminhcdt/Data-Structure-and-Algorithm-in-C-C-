 ...
 * 
 * @example Given N = 1041 the function should return 5, because N has binary representation 10000010001 and so its longest binary gap is of length 5. Given N = 32 the function should return 0, because N has binary representation '100000' and thus no binary gaps.
 * 
 * @note Assumptions:
...
int solution(int N) {
  ...
  int input_value = N;
  while (input_value != 0) {
    if (input_value & 0x1) binary.push_back(count);
    input_value >>= 1;
    ++count;
  }

  int max = 0;
  for(unsigned int i = 1; i < binary.size(); ++i) {
    int gap = binary[i] - binary[i - 1] - 1;
    if (gap > max) max = gap;
  }
