/***
 * 
 * @brief [Task description](https://app.codility.com/programmers/trainings/9/sparse_binary_decomposition/)
 * 
 * A non-negative integer N is called sparse if its binary representation does not contain two consecutive bits set to 1.
 * For example, 41 is sparse, because its binary representation is "101001" and it does not contain two consecutive 1s. On the other hand, 26 is not sparse, because its binary representation is "11010" and it contains two consecutive 1s.
 ...
 * 
 * @example Given N = 26 the function may return 8, 9, 17 or 18, as explained in the example above. All other possible results for N = 26 are 5, 10, 16 and 21.
 * 
 * @note Assumptions:
...
bool is_parse(int value) {
  int prev = 0;
  while (value != 0) {
    if ((prev == 1) && ((value & 0x01) == 1)) return false;
    prev = value & 0x01;
    value >>= 1;
  }
...
int solution(int N) {
  ...
  for (int i = N / 2; i >= 0; --i) {
    if (is_parse(i) && is_parse(N - i)) return i;
  }
